...
class ActionMailer::ARSendmail2
...
#{Dir.pwd} does not appear to be a Rails application root.
  ...
  def cleanup
    ...
    mail = @email_class.find(:all, :conditions => conditions) 
    log "expiring #{mail.length} emails from the queue, max age #{@max_age}"
    mail.each do |em|
      em.on_purge(@max_age) if em.respond_to? :on_purge # PFZ
      em.destroy
    end
  end
  ...
  def deliver(emails)
    ...
    Net::SMTP.start smtp_settings[:address], smtp_settings[:port],
      ...
      until emails.empty? do
        ...
        email_to = email.to
        begin
          res = smtp.send_message email.mail, email.from, email_to
          email.on_success(res) if email.respond_to? :on_success # PFZ
          email.destroy
          ...
          email.on_fatal(e) if email.respond_to? :on_fatal # PFZ
          email.destroy
          smtp.reset
    ...
    log "server busy or other error"
